(function ($, window) {
    ...
    function createResourceChosen($select, resources, cv, width)
    {
        ...
        {
            Hualala.Global.getWeixinResources({isActive: 1, resType: '(0,1,2)'}, function(rsp)
            {
                if(rsp.resultcode != '000')
                {
                    rsp.resultmsg && U.TopTip({msg: rsp.resultmsg, type: 'danger'});
    ...
    {
        var emotions = emotions || getEmotions(),
            resType = res.resType;
