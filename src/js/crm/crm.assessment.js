(function ($, window) {
    ...
    var G = Hualala.Global,
       ...
       var queryFormTpl,ModalTpl,listTpl,itemTpl,Assessments = null,params;
       var $pager = $('<div>').addClass('pull-right');
    Hualala.CRM.initAssessment = function ($container){
    ...
    function initTemplate($container) {
        queryFormTpl =  Handlebars.compile(Hualala.TplLib.get('tpl_assessment_query_panel'));
        listTpl = Handlebars.compile(Hualala.TplLib.get('tpl_assessment_list'));
        ModalTpl= Handlebars.compile(Hualala.TplLib.get('tpl_feedBack_Modal'));
        itemTpl = Handlebars.compile(Hualala.TplLib.get('tpl_assessment_card'));
        Handlebars.registerPartial("assessmentCard", Hualala.TplLib.get('tpl_assessment_card'));
    ...
    function renderQueryBox($container){
        ...
        U.createSchemaChosen($('[name=shopName]'), $('[name=cityID]'));
    }
    //获取数据
    function renderTable($container,params) {
       G.getAssessment(params, function(rsp) {
            if (rsp.resultcode == '000') {
                Assessments = rsp.data.records;
                var page = rsp.data.page;              
                renderRecords(Assessments,$container); 
    ...
    function mapTimeData(s) {
        ...
        if (IX.isString(s) && s.length > 0) {
            ...
            s1 = IX.Date.getDateByFormat(s1, 'yyyy-MM-dd HH:mm:ss');
    ...
    function mapRenderData(records) {
        var ret = _.map(records, function (record, i, l) {
            var clientTypeVal = $XP(record, 'clientType', ''),
                feedbackVal = $XP(record,'feedbackType'),
                responseVal = $XP(record,'responseStatus');
            ...
            var createTimeVal = $XP(record,'createTime'),
                actionTimeval = $XP(record,'actionTime');
            ...
            return {
                ...
                shopName :$XP(record,'shopName'),
                shopUrl: Hualala.Common.getShopUrl(record.shopID),
                clientType : $XP(clientTypeVal[0], 'label', ''),
                feedbackType : $XP(feedbackVal[0], 'label', ''),
                orderKey : $XP(record, 'orderKey', ''),
                responseStatus : $XP(responseVal[0], 'label', ''),
                feedbackContent : $XP(record,'feedbackContent'),
                responseContent : $XP(record,'responseContent'),
                operator : $XP(record,'operator'),
                userName : $XP(record,'userName'),
                userMobile : $XP(record,'userMobile'),
                createTime : createTimeVal,
                actionTime : actionTimeval,
                hiddenFooter : $XP(record,'responseStatus')==0?"hidden" :"",
                hiddenNooperate : $XP(record,'responseStatus')==0?"" :"hidden",
                hiddenoperate : $XP(record,'responseStatus')==1?true :false,
            };
        });
        return {
            assessmentCard : {
    ...
    function bindEvent($container){
        $container.on('click', '.operate,.btn,.orderFoodDetail', function (e) {
            ...
            switch(act) {
                ...
                case "detail":
                    var $btn = $(this),
                        orderKey = $btn.attr('data-href'),
                        transType = '101';
                        orderdetail($btn,orderKey,transType);
                    break;
            }
        });
    }
    //订单详情
    function orderdetail($btn,orderKey,transType){
    ...
    function renderDailog(itemID,act,$container){
        var feedItem ={};
            feedItem=_.findWhere(Assessments, {itemID: itemID});
