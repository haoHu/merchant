(function ($, window) {
	...
	var QueryPromotionShopView = Stapes.subclass({
	...
	QueryPromotionShopView.proto({
		...
		getQueryParams : function () {
			// TODO
			var self = this;
			var shopName = self.shopName || null;
			var params = self.getFilterParams();
			var shops = self.model.getShops();
			if (IX.isEmpty(shopName)) {
				shopName = '';
			} else {
				shopName = $XP(_.find(shops, function (el) {
					return $XP(el, 'shopID') == shopName;
				}), 'shopName', '');
			}
	...
	Hualala.Shop.QueryPromotionShopView = QueryPromotionShopView;
})(jQuery, window);