(function ($, window) {
	...
	var QueryPromotionShopModel = Stapes.subclass({
		constructor : function () {
			...
			// 数据是否已经加载完毕
			this.isReady = false;
			this.callServer = Hualala.Global.getAllowRefPromotionShop;
		}
	});
	QueryPromotionShopModel.proto({
		// 数据模型初始化，获取查询条件的数据
		init : function (params, cbFn) {
			...
			self.callServer(params, function (res) {
				if (res.resultcode == '000') {
					// 装填原始数据
					self.origCities = $XP(res, 'data.datasets.cityCountDs.data.records', []);
					self.origAreas = $XP(res, 'data.datasets.areaCountDs.data.records', []);
					self.origShops = $XP(res, 'data.records', []);
					self.initDataModel();
					self.isReady = true;
				} else {
					toptip({
			...
			_.each(this.origCities, function (c, i, l) {
				var cid = $XP(c, 'cityID'), 
					...
					areaLst = _.filter(self.origAreas, function (a) {
						return $XP(a, 'areaID').slice(0,4) == cid;
					});
	...
	Hualala.Shop.QueryPromotionShopModel = QueryPromotionShopModel;
})(jQuery, window);